...
int main(int argc, char **argv) {
  ...
  start = omp_get_wtime();
  tinit = start;
  magma_setup_kron_cpuv1(*C, l_patch_size, r_patch_size);
  tsetup = omp_get_wtime()-tinit;

  tinit = omp_get_wtime();
  int trials = 1;
  for (int num = 0; num < trials; num++) {
    magma_apply_Htarget_cpuv1(*C, l_patch_size, r_patch_size, X, Y);
    #if defined(MAGMA_CHECK_ERROR)
    ...
    printf("    Magma_CPU-ref error is %10.5e\n",kron_error/Ynorm);
  ...
  magma_destroy_kron_cpuv1(*C, l_patch_size, r_patch_size);
