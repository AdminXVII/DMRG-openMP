...
int apply_Htarget(Block_Matrix_t &CIJ, 
  ...
  Kokkos::MemoryPool<Kokkos::OpenMP> memPool (25 * max, 32, max);
     Kokkos::parallel_for( team_policy(npatches, 1 ), [&](const member_type &teamMember){
//     for (int ipatch = 0; ipatch < npatches; ipatch++){
          const int ipatch = teamMember.league_rank();
	  const int i1 = vstart[ipatch];
            ...
            //team Sharing -- team Captain should initialize 
            double *YI = memPool.allocate (vsize[ipatch]);
          ...
          **/
          ...
          for(int jpatch = jpatch_start; jpatch < jpatch_end; jpatch++){
             ...
             const int j2 = j1 + vsize[jpatch]; 
              ...
              double *XJ = memPool.allocate(size_XJ);              
              ...
              double *YIJ = memPool.allocate(vsize[ipatch]);
              ...
              for(int k = 0; k < size_list_k; k++){
                  ...
                  double *Y_return = memPool.allocate(vsize[ipatch]);                  
