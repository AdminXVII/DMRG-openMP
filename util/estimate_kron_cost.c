                         ...
                         int *p_imethod )
{
  ...
  % ------------------------
  */
  double flops_BY = (two*nnz_B)*ncol_A;
  double flops_BYAt = (two*nnz_A)*nrow_B;
  ...
  % ------------------------
  */
  double flops_YAt = (two*nnz_A)*ncol_B;
  flops_BYAt = (two*nnz_B)*nrow_A;
  ...
  % ------------------
  */
  double flops_method3 =  (two*nnz_A)*nnz_B;
  ...
  if (minimize_flops) {
    ...
    if (kron_flops == flops_method3) {
        imethod = 3;
        };
    assert( (1 <= imethod) && (imethod <= 3));
   }
  else {
    /*
     * ------------------------------------
